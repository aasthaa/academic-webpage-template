...
googleCite = function(theurl, theauthor, plotIt = TRUE, pdfname=NULL, unpublishedArticles) {
  ...
  if(plotIt) {
    ...
    if (!is.null(pdfname)) pdf(pdfname, h = 6, w = 10)
    
    #par(mfrow = c(1,2))
    #makeAuthorCloud(alldata)
    makePaperCloud(alldata, unpublishedArticles)
...
makePaperCloud = function(tab, unpublished) {

  unpublishedTitles <- read.csv("unpublished")
...
makePaperCloud_manual = function(published, unpublished, filename) {

  publishedWords <- read.csv(published, header=T)
  publishedWords <- as.character(unlist(publishedWords[,-1]))
  publishedWords <- as.vector(unlist(sapply(publishedWords, function(x) strsplit(x, split=","))))

  unpublishedWords <- read.csv(unpublished, header=T)
  unpublishedWords <- as.character(unlist(unpublishedWords[,-1]))
  unpublishedWords <- as.vector(unlist(sapply(unpublishedWords, function(x) strsplit(x, split=","))))

  words <- c(publishedWords, unpublishedWords)
...
#            random.order = FALSE, colors = pal,vfont=c("sans serif","plain"))
  ...
  #pdf(pdfname, h = 6, w = 10)
  png(filename, width=12,height=8, units='in', res=400)
     wordcloud(words = d$word, freq = d$freq, 
            min.freq = 1, max.words = Inf,
            random.order = FALSE, colors = pal)
  dev.off()
